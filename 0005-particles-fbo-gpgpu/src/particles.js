...
gui.add(debugObject, 'radius').min(0.1).max(3).step(0.1);
gui.add(debugObject, 'intersectRadius').min(0.01).max(3).step(0.01);
gui.add(debugObject, 'particleCount').min(10).max(100).step(10);
...
const startParticleMove = () => {
  ...
  if(activeParticleIndex >= particleCount) {
    console.log('Last Particles Finished');
    return; 
  }

  const positions = particles.geometry.attributes.position.array;
  let i3 = activeParticleIndex * 3;

  // Spawn 
  const startPosition = getParticleStartPostion(activeParticleIndex);
...
const updateParticleState = () => {
  // update
  particleState.activeParticleIndex += 1;
  particleState.needsNewParticle = false;
  particleState.fallTime = 0;
...
const detectCollision = () => {
  const {
    activeParticleIndex: activeFlakeIndex,
  } = particleState;
  ...
  for (let i = 0; i < activeFlakeIndex; i++) {
      ...
      if(checkIntersect(flakePosition, testPosition)) {
        ...
        particleState.needsNewParticle = true;
...
const tick = () =>
{
  ...
  if(debugObject.isMoving) {
    particleState.fallTime += deltaTime;
  ...
  if(particleState.needsNewParticle) {
    updateParticleState();
  }
